...
tf.compat.v1.flags.DEFINE_integer("embedding_dim", 128, "Dimensionality of character embedding (default: 128)")
tf.compat.v1.flags.DEFINE_string("filter_sizes", "2,3,5", "Comma-separated filter sizes (default: '2,3,5')")#3,4,5 ##2,4,5
tf.compat.v1.flags.DEFINE_integer("num_filters", 120, "Number of filters per filter size (default: 128)")#128
...
tf.compat.v1.flags.DEFINE_integer("num_epochs",10, "Number of training epochs (default: 200)")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_batch, learning_rate):
            ...
            with open(os.path.join(ROOT_DIR, "train/output/Optimized-Text-CNN-Nadamax.txt"), 'a') as f:
                # نوشتن متن جدید به فایل 
                f.write("{},{:g},{:g},{:g}\n"
                  .format(step,loss,accuracy,learning_rate)) 
            # بستن فایل و ذخیره تغییرات
            f.close() 
            # print(os.path.join(ROOT_DIR, "train/output/DUAL-optimizer.txt")) 
            # print("ذخیره شد!"
